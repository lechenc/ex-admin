<template>
  ...
  >
    ...
    <template v-for="config in configs">
      ...
      <el-table-column
        v-if="config.type === 'noneShowValue'"
        :key="config.prop"
        :label="config.label"
        :width="config.width ? config.width : ''"
        :min-width="90"
      ...
      >
        <template slot-scope="scope">
          <span v-if="scope.row[config.prop]">
            {{ scope.row[config.prop] + config.jointValue }}
          </span>
          <span v-else> - </span>
        </template>
      </el-table-column>
      ...
      <!-- 最多保留8位小数 -->
      <el-table-column
        v-if="config.type == 'onlyEightDecimals'"
        :key="config.prop"
        :label="config.label"
        :width="config.width ? config.width : ''"
        :min-width="120"
      ...
      <!-- 减一个其他值 -->
      <el-table-column
        v-if="config.type == 'minusOthersNumber'"
        :key="config.label"
        :label="config.label"
        :width="config.width ? config.width : ''"
        :min-width="120"
      >
        <template slot-scope="scope">
          <span v-if="config.othersNumber">
            {{ getMinus(scope.row[config.prop], config.othersNumber) }}
          </span>
      ...
      >
        <template slot-scope="scope">
          ...
          <span
            ...
            :style="{
              color:
                parseInt(scope.row[config.prop2]) == 0
                  ? '#4379FF'
                  : parseInt(scope.row[config.prop2]) == 2
                  ? '#000'
                  : '#EABF40'
            }"
          >
            {{ (scope.row[config.prop] / 60 / 60).toFixed(2) + '小时' }}
          </span>
        </template>
      </el-table-column>

      <el-table-column
        v-if="config.type === 'textColorTwo'"
        :key="config.prop"
        :prop="config.prop"
        :label="config.label"
        :width="config.width ? config.width : ''"
      ...
      >
        <template slot-scope="scope">
          <span class="td-color" :style="{ color: '#4379FF' }">{{
            (scope.row[config.prop] / 60 / 60).toFixed(2) + '小时'
          }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        v-if="config.type === 'textAndfilter'"
        :key="config.prop"
        :label="config.label"
        :width="config.width ? config.width : ''"
        :min-width="120"
      >
        <template slot-scope="scope">
          <span v-if="config.show_type === 'text'">{{
            scope.row[config.prop] | textFilter(config.filters)
          }}</span>
          <span>+</span>
      ...
      >
        <template slot-scope="scope">
          <span
            class="td-color"
            :style="{ color: parseInt(scope.row[config.prop2]) == 0 ? '#000' : '#4DA4FF' }"
          >
            {{ scope.row[config.prop] }}
          </span>
        </template>
      ...
      <el-table-column
        v-if="config.type === 'positiveColor'"
        :key="config.prop"
        :prop="config.prop"
        :label="config.label"
        :width="config.width ? config.width : ''"
      ...
      >
        <template slot-scope="scope">
          <span
            class="td-color"
            :style="{ color: parseInt(scope.row[config.oldProp]) > 0 ? '#EABF40' : '#000' }"
          >
            {{ scope.row[config.prop] }}
          </span>
        </template>
        ...
        <template slot-scope="scope">
          <div>
            <span v-for="(item, ixx) in config.arr" :key="ixx">
              {{ scope.row[item] }}
              <span v-if="ixx != config.arr.length - 1" style="margin-right: 5px">
                {{ config.join }}
              </span>
            </span>
          </div>
      ...
      >
        <template slot-scope="scope">
          <span v-if="parseInt(scope.row[config.prop]) > 0">{{
            scope.row[config.propArr[0]] + scope.row[config.propArr[1]]
          }}</span>
      ...
      >
        <template slot-scope="scope">
          <div v-if="config.typeDetail === 'numlist'" class="colline">
            <span v-for="(item, idx) in scope.row[config.prop]" :key="idx">
              {{ item[config.label1] }}
            </span>
          </div>
          <div v-else-if="config.typeDetail === 'stringlist'" class="colline">
            <span v-for="(item, idx) in scope.row[config.prop]" :key="idx">
              {{ item }}
            </span>
          </div>
          <div v-else-if="config.typeDetail === 'stringlistMore'" class="colline">
            <span v-for="(item, idx) in scope.row[config.prop]" :key="idx">
              {{ item[config.arrProp[0]] }} - {{ item[config.arrProp[1]] }} -
              {{ item[config.arrProp[2]] }}
            </span>
          </div>

          <!-- 能根据逗号拆分为数组后跳转 -->
          <div v-else-if="config.typeDetail === 'dotStrList'" class="colline">
            <template v-if="(scope.row[config.prop] + '').indexOf(',') > 0">
              <el-button
                v-for="(item, idx) in (scope.row[config.prop] + '').split(',')"
                :key="idx"
                style="margin-bottom: 4px"
                plain
                type="primary"
                @click="doHandleLine($event, scope.row, config.fn, item)"
              >
                {{ item }}
              </el-button>
            </template>
            <template v-else-if="scope.row[config.prop]">
              <el-button
                plain
                type="primary"
                @click="doHandleLine($event, scope.row, config.fn, scope.row[config.prop])"
              >
                {{ scope.row[config.prop] }}
              </el-button>
            </template>
          ...
          <!-- 能根据逗号拆分为数组后不跳转 -->
          <div v-else-if="config.typeDetail === 'dotStrListNoClick'" class="colline">
            <template v-if="(scope.row[config.prop] + '').indexOf(',') > 0">
              <span
                v-for="(item, idx) in (scope.row[config.prop] + '').split(',')"
                :key="idx"
          ...
          <div v-else class="colline">
            <span v-for="(item, ix) in scope.row[config.prop]" :key="ix">
              {{ item[config.label1] }}: {{ Math.floor(item[config.label2] * 10000) / 100 }}%
            </span>
          </div>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <div>
            <span>{{ scope.row[config.arr[0]] }}</span> -
            <span>{{ scope.row[config.arr[1]] }}</span>
          </div>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <div class="img-wraper">
            <img
              ...
              alt=""
              @click="viewImg(scope.row[config.prop])"
            />
            <span v-else>无</span>
          </div>
        </template>
        ...
        <template slot-scope="scope">
          <div class="img-wraper">
            <span
              v-if="scope.row[config.prop]"
              class="video-txt"
              @click="viewVideo(scope.row[config.prop])"
            >
              播放视频
            </span>
            <span v-else>无</span>
      ...
      <el-table-column v-if="config.type === 'imgArr'" :key="config.prop" :label="config.label">
        <template slot-scope="scope">
          <div v-if="(scope.row[config.prop] + '').indexOf(',') > 0" class="colline">
            <img
              v-for="(item, idx) in scope.row[config.prop].split(',')"
              :key="idx"
              title="单击大图预览"
              ...
              :height="'34px'"
              style="margin-bottom: 4px"
              :src="scope.row[config.prop].split(',')[idx]"
              alt=""
              @click="viewImg(scope.row[config.prop].split(',')[idx])"
            />
          </div>
      ...
      <!-- qrcode图 特定 -->
      <el-table-column
        v-if="config.type === 'qrimg'"
        :key="config.prop"
        :label="config.label"
        :width="config.width ? config.width : '210px'"
      >
        <template slot-scope="scope">
          <div class="qrimg-wraper">
            ...
            <div v-else>
              <p
                ...
                class="rowLine"
                @click="doHandle($event, scope.row, config.fn)"
              >
                <span style="min-width: 90px; margin-right: 4px">{{ scope.row[config.prop] }}</span>
                <img
                  style="width: 20px; position: relative; margin-right: 3px"
      ...
      <el-table-column
        v-if="config.type === 'filter_myShowHide'"
        :key="config.prop"
        :label="config.label"
        :width="config.width ? config.width : ''"
        :min-width="120"
      ...
      >
        <template slot-scope="scope">
          ...
          <el-tag
            v-else-if="config.show_type === 'tag' && config.prop"
            :type="scope.row[config.prop] | typeFilter(config.filters)"
          >
            {{ scope.row[config.prop] | textFilter(config.filters) }}
          </el-tag>
          <span
            v-if="config.show_type === 'textColorLine'"
            :style="{
              color:
                scope.row[config.prop] == 1 && scope.row[config.other] != 3 ? '#C5012F' : '#000'
            }"
          >
            {{ scope.row[config.prop] | textFilter(config.filters) }}
          </span>
          <span v-if="config.show_type === 'filters' && config.filters === 'optTypeFB'">{{
            scope.row[config.prop] | optTypeFB
          }}</span>
          <span v-if="config.show_type === 'filters' && config.filters === 'matchResultMap'">{{
            scope.row[config.prop] | matchResultMap
          }}</span>
        </template>
      </el-table-column>
      ...
      <!-- 开关 -->
      <el-table-column
        v-if="config.type === 'switch' && actionShow"
        :key="config.prop"
        :label="config.label"
        :width="config.width ? config.width : ''"
      >
      ...
      <!-- 操作按钮 -->
      <el-table-column
        v-if="config.type === 'action' && actionShow"
        :key="config.prop"
        fixed="right"
        :label="config.label"
        :width="isDeskTop ? config.width : '120px'"
      >
        <template slot-scope="scope">
          <span v-for="(btn, i) in config.btnGroup" :key="i + 't`t'">
            ...
            <el-button
              v-else-if="
                !btn.filter_key && !btn.filter_type && btn.type && !btn.other_filter && !btn.out
              "
              :type="btn.type"
              plain
              size="small"
            ...
            <template
              v-else-if="
                btn.filter_type == 'filter_indexOf_Except' && indexOfExceptFn(scope.row, btn)
              "
            >
              <el-button
                slot="reference"
          ...
          <template v-if="actionMoreShow">
            <el-dropdown type="primary" class="more_dropdown">
              <el-button type="primary" plain>
                {{ actionMoreText }}<i class="el-icon-arrow-down el-icon--right"></i>
              </el-button>
              <el-dropdown-menu slot="dropdown">
                ...
                >
                  <el-button
                    v-if="
                      !item.filter_type || !item.filter_status || indexOfExceptFn(scope.row, item)
                    "
                    plain
                    type="primary"
...
export default {
  ...
  methods: {
    ...
    headClass({ row, column, rowIndex, columnIndex }) {
      if (this.headColor) {
        const color = this.configs[columnIndex].headColor
        return `color :${color}`
      }

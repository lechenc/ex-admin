<template>
	...
	<el-dialog class="addUser-page" title="增加码商" :visible.sync="showWidget">
		<el-form :model="filterForm" label-width="95px" size="small" inline>
...
export default {
	...
	methods: {
		...
		async getUserData (saerch) {
			this.filterForm.groupId = this.groupId
			const res = await $api.findInviteChildByGroup(this.filterForm);
			if (res && res.data.code == 200) {
				let list = []
				if (saerch == 'saerch' && res.data.result.userId) {
		...
		async load (tree, treeNode, resolve) {
			...
			const params = {
				inviteCode: inviteCode, groupId: this.groupId
			}
			const res = await $api.findInviteChildByGroup(params);
			if (res) {

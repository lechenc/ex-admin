  ...
  <div class="setCheckstand">
    ...
    <el-form ref="defaultForm" :model="defaultForm" :inline="true" label-width="140px" size="small" class="defaultForm">
      <el-form-item label="开启单独设置：">
        <el-switch v-model="defaultForm.CHECK_STAND_SWITCH" active-color="#13ce66" @change="changeSetting(8)"> </el-switch>
      </el-form-item>
      ...
      <el-form-item label="收银台支付入口：" v-if="defaultForm.CHECK_STAND_SWITCH">
        <el-checkbox-group v-model="defaultForm.CHECK_STAND_PAY_TYPE" :min="1" @change="changeSetting(5)">
...
export default {
  ...
  methods: {
    ...
    async changeSetting(type) {
      ...
      const res = await $api.updateBatchOutFee(params);
      if (res) {
        this.$notify.success({
          title: "提示",
          message: "修改收银台设置成功",

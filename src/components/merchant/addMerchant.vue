<template>
	...
	<el-dialog class="addMerchant-page" title="增加商户" :visible.sync="showWidget">
		<el-form :model="merchantFormData" label-width="105px" size="small" inline>
		...
		<el-table
			class="addMerchantSelect"
			@header-click="headerClick"
			:data="merchantPageData.list"
			height="420px"
			border
			size="small"
			style="min-width: 100%"
	...
	methods: {
		...
		async addMerchantToGroup () {
			...
			if (this.groupId !== '') {
				...
				const res = await $api.addMerchantToGroup(params);
		...
		async getMerchantList () {
			...
			const res = await $api.listByGroup(this.merchantFormData);
			if (res && res.data.code == 200) {
				// this.pageData = res.result.page
				this.merchantPageData = res.data.result.page

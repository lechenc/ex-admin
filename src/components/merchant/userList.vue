<template>
  <div class="userList-page">
    <div class="title">
      <span>码商列表：</span>
      <p>
        <el-button size="small" type="danger" :disabled="multipleSelection.length == 0" @click="checkedDelete()"> 一键删除 </el-button>
        <el-button size="small" type="primary" @click="$refs.AddUser.show(list, groupId)"> 增加码商 </el-button>
      </p>
    </div>
    <el-table :data="list" height="auto" border size="small" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="date" label="操作" fixed="right" width="110" align="center">
        <template slot-scope="scope">
          <el-button size="small" type="text" @click="subUserToGroup(scope.row.userId)"> 删除 </el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    ...
    async getData() {
      const res = await $api.checkUserGroupRecdInfo(this.pageData);
      if (res) {
      ...
      if (this.groupId !== "") {
        this.$confirm("此操作将会将用户移出该分组，确定删除？", "提示", {
          ...
          .then(async () => {
            ...
            const res = await $api.subUserToGroup(params);

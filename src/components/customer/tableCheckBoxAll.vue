      ...
      <el-form :model="searchForm" ref="searchForm">
        <el-row>
          <el-col :span="12">
            ...
            <el-form-item label="商户编号" prop="id222">
              <el-input type="text" placeholder="请输入" v-model="searchForm.id222"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-button type="primary" size="small" @click="searchDiaLogList">搜索</el-button>
            <el-button type="primary" size="small" @click="confirmOp('yes')">通过</el-button>
            <el-button type="primary" size="small" @click="confirmOp('no')">不通过</el-button>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    async getListAll(val) {
      ...
      const res = await $api.aaa(params);
      if (res) {
    ...
    // 弹出框里列表筛选条件
    searchDiaLogList() {
      if (this.$util.isEmptyObject(this.searchForm)) {
        this.getListAll('search');
      }else{
        this.$message({type:"warning",message:"请输入筛选条件"})
      }
    },
    // 最终确定增加的数据,返回给列表
    confirmOp(str) {
      ...
      if (tmp) {
        this.$emit('handler', { arr: tmp, type: str });
      } else {

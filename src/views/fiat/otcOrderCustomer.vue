...
import { otcOrderCustomerCol, otcOrderCustomerColNoBtn, otcOrderCustomerConfig } from '@/config/column/fiat';
      ...
      if (res) {
        ...
        if (getObj) {
          ...
          if (this.search_params_obj.advertType || this.search_params_obj.advertType == 0) {
            const tm1 = this.searchCofig[7]['list'].filter(v => v.value == this.search_params_obj.advertType)[0].label;
            temp += `<p>广告类型：${tm1}</p>`;
          }

 ...
 * @LastEditTime: 2020-12-01 17:40:00
...
import { otcOrderCol, otcOrderColNoBtn, otcOrderConfig } from '@/config/column/fiat';
  ...
  methods: {
    async doHandle(data) {
      ...
      if (fn === 'dialog') {
        ...
        this.$router.push({ path: '/fiat/dialogueRecord' });
      }
    },
    doSearch(data) {
      this.current_page = 1;
      ...
      if (res) {
        ...
        if (getObj) {
          ...
          if (this.search_params_obj.advertType || this.search_params_obj.advertType == 0) {
            const tm1 = this.searchCofig[6]['list'].filter(v => v.value == this.search_params_obj.advertType)[0].label;
            temp += `<p>广告类型：${tm1}</p>`;
          }
    ...
    async getList() {
      ...
      if (res) {
        ...
        records.forEach((v) => {
          v['isclick'] = parseInt(v['tradeStatus']) == 4 || parseInt(v['tradeStatus']) == 7 || parseInt(v['tradeStatus']) == 8 ? false : true;
        });
        this.total = +total;
        this.pages = +pages;

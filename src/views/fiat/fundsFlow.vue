 ...
 * @LastEditTime: 2020-08-06 14:46:30
...
import { fundsFlowCol, fundsFlowColNoBtn, fundsFlowConfig } from '@/config/column/fiat';
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await $api.queryOtcMoneyChangeInfo(query_data);
      if (res) {
    ...
    requiredParams(params) {
      ...
      if (this.search_params_obj.coinName) {
        this.search_params_obj.coinName = this.coinList.filter(v => v.value == this.search_params_obj.coinName)[0].label;
      }
    },
  },
  mounted() {
    this.configs = fundsFlowCol;
    this.searchCofig = this.$util.clone(fundsFlowConfig);

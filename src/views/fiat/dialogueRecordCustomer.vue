 ...
 * @LastEditTime: 2020-09-28 17:02:30
...
<template>
  <div class="dialogueRecordCustomer-container">
            ...
            <div class="talk">
              ...
              >
                <div class="txt-line">
                  ...
                  <div class="tip-dot" v-show="item.type == 1" :class="item.haveRead != 1 ? 'red' : 'white'"></div>
                  <div class="inner-txt" v-if="item.type == 1 || item.type == 3">{{ item.content }}</div>
                  <div class="inner-txt" v-else-if="item.type == 2">
                    <viewer :options="options"><img :src="item.content" alt=""/></viewer>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  activated() {
    ...
    this.words = ''; //发送的字符
    this.orderId = '';
    this.row = JSON.parse(localStorage.getItem('dialogueRowCustomer'));
  ...
  deactivated(){
    localStorage.removeItem('dialogueRowCustomer')
  },
  beforeDestroy() {
...
.dialogueRecordCustomer-container {

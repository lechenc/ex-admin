 ...
 * @LastEditTime: 2020-07-20 18:35:06
  ...
  <div class="invitation-container">
    ...
    <div class="container-middle">
      ...
      >
        ...
        <el-table-column align="center" label="操作" width="80" v-if="isCURDAuth">
          <template slot-scope="scope">
            <el-button type="text" size="mini" @click="openDialogCheck(scope.row)">迁移</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog :visible.sync="dialogEditVisible" width="700px" :title="title">
      <el-form :model="form" ref="form" :rules="rules">
        <el-form-item label="兑入佣金费率" prop="aaa">
          <el-select v-model="form.aaa" placeholder="" wdith="20%" @change="changeCashIn">
            <el-option v-for="(item, idx) in cashList" :key="idx" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <div v-if="form.aaa === 1">
          <el-form-item label="银行卡" prop="ccc">
            <el-input type="text" placeholder="请输入" v-model="form.ccc">
              <div slot="append" class="gcode">%</div>
            </el-input>
          </el-form-item>
          <el-form-item label="支付宝" prop="ddd">
            <el-input type="text" placeholder="请输入" v-model="form.ddd">
              <div slot="append" class="gcode">%</div>
            </el-input>
          </el-form-item>
          <el-form-item label="微信" prop="eee">
            <el-input type="text" placeholder="请输入" v-model="form.eee">
              <div slot="append" class="gcode">%</div>
            </el-input>
          </el-form-item>
        </div>
        <div v-else>
          <el-form-item label="购买佣金费率" prop="bbb">
            <el-input type="text" placeholder="请输入" v-model="form.bbb">
              <div slot="append" class="gcode">%</div>
            </el-input>
          </el-form-item>
        </div>

        <el-form-item label="兑出佣金费率" prop="aaa">
          <el-select v-model="form.bbb" placeholder="" wdith="20%" @change="changeCashOut">
            <el-option v-for="(item, idx) in cashList" :key="idx" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <div v-if="form.aaa === 1">
...
import { inviteCol, inviteColNoBtn, invitationConfig } from '@/config/column/fiat';
...
export default {
  ...
  methods: {
    ...
    async getList(str) {
      ...
      if (res) {
        let list = [];
        res = res.data;
        if (str && str === 'saerch' && res.result.userId) {
          list = [res.result];
  ...
  mounted() {
    let authObj = this.$util.getAuthority('FiatInvitation', inviteCol, inviteColNoBtn);
    ...
    this.searchCofig = this.$util.clone(invitationConfig);
    this.getList();
  },
};
</script>

...
import { appealCol, appealColNoBtn, appealConfig } from '@/config/column/fiat';
...
export default {
  ...
  methods: {
    ...
    async doHandle(data) {
      ...
      this.row = row;
      if (fn === 'detail') {
        this.$router.push({ path: '/fiat/appealJudge', query: { label: 'detail', id: row.id, appealStatus: row.appealStatus } });
      }
      if (fn === 'edit') {
        this.$router.push({ path: '/fiat/appealJudge', query: { label: 'edit', id: row.id, appealStatus: row.appealStatus } });
      }
      if (fn === 'judgment') {
        this.$router.push({ path: '/fiat/appealJudge', query: { label: 'judgment', id: row.id, appealStatus: row.appealStatus } });
      }
    },
    doSearch(data) {
      this.current_page = 1;

<template>
  <div class="recharge-container">
    ...
    <div class="container-btn" v-if="isCURDAuth">
      <el-button type="primary" size="medium" @click="goPage">创建分组</el-button>
    </div>
    <div>
...
import { groupAuthCol, groupAuthColNoBtn, groupAuthConfig } from "@/config/column/merchant";
...
export default {
  ...
  methods: {
    doHandle(data) {
      const { fn, row } = data;
      if (fn === "edit") {
        this.$router.push({path:'/merchant/groupAuthDetail',query:{id:row.recdId,createTime:this.formatTime(row.createTime),groupName:row.groupName}});
      } else if (fn === "del") {
    ...
    goPage() {
      this.$router.push({ name: "GroupAuthDetail" });
    },
    // getlist
    async getList() {
  ...
  mounted() {
    // let authStatus = this.$util.getAuthority("GroupAuth");
    // if (authStatus === 2) {
    //   this.configs = groupAuthColNoBtn;
    //   this.isCURDAuth = false;
    // } else {
    //   this.configs = groupAuthCol;
    // }
    this.configs = groupAuthCol;
    this.searchCofig = groupAuthConfig;
    this.getList();
  },
};
</script>

<template>
  <div class="contractInOutGold-container">
...
import { contractInOutGoldCol, contractInOutGoldConfig } from '@/config/column/inOutGold';
...
export default {
  ...
  methods: {
    ...
    async calTotal(data) {
      ...
      const res = await $api.getContractInOutGoldTotal(params);
      if (res) {
    ...
    async getList() {
      ...
      const res = await $api.getContractInOutGoldList(params);
      if (res) {
  ...
  mounted() {
    this.configs = contractInOutGoldCol;
    this.searchCofig = this.$util.clone(contractInOutGoldConfig);
    // 初始化今天，和前天的时间

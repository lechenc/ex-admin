<template>
  <div class="coinInOutGold-container">
...
import { coinInOutGoldCol, coinInOutGoldConfig } from '@/config/column/inOutGold';
...
export default {
  ...
  methods: {
    ...
    async calTotal(data) {
      ...
      const res = await $api.getCoinInOutGoldTotal(params);
      if (res) {
    ...
    async getList() {
      ...
      const res = await $api.getCoinInOutGoldList(params);
      if (res) {
  ...
  mounted() {
    this.configs = coinInOutGoldCol;
    this.searchCofig = this.$util.clone(coinInOutGoldConfig);
    // 初始化今天，和前天的时间

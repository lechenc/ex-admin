 ...
 * @LastEditTime: 2020-12-17 21:20:25
...
<template>
  <div class="coinContract-container">
    <div class="container-top">
      <Bsearch :estimate="true" @do-estimate="estimate" :configs="searchCofig" @do-search="doSearch" @do-reset="doReset" />
    </div>
    <div>
    ...
    <el-dialog title="计算结果" class="estimateDialog" :visible.sync="centerDialogVisible" width="40%">
      ...
      <div class="estimateBtn">
        <div>
          <el-button @click="check(1)" type="primary" :disabled='!!!jform.numberStock'>查看</el-button>
        </div>
        <div>
          <el-button @click="check(2)" type="primary"  :disabled='!!!jform.numberProfit'>查看</el-button>
        </div>
        <div>
          <el-button @click="check(3)" type="primary" :disabled='!!!jform.numberLoss'>查看</el-button>
        </div>
      </div>
    </el-dialog>
  </div>
...
import { contractEstimateCol, contractEstimateConfig } from '@/config/column/contract';
...
export default {
  ...
  methods: {
    ...
    check(val) {
      let userId;
      if (val == 1) {
        userId = this.dataObj.numberStockUserId || [];
      } else if (val == 2) {
        userId = this.dataObj.numberProfitUserId || [];
      } else if (val == 3) {
        userId = this.dataObj.numberLossUserId || [];
      }
      let price = this.price;
      let coinId = this.dForm.coinId;
      this.dialogFormVisible = false;
      this.centerDialogVisible = false;
      this.$router.push({
        path: '/contract/risk/contractResult',
        query: {
          topBtn: true,
  ...
  async mounted() {
    this.configs = contractEstimateCol;
    this.searchCofig = this.$util.clone(contractEstimateConfig);

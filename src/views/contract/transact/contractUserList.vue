...
import { contractUserListCol, contractUserListConfig } from '@/config/column/contract';
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await $api.getContractUserList(query_data);
      if (res) {
  ...
  mounted() {
    this.configs = contractUserListCol;
    this.searchCofig = this.$util.clone(contractUserListConfig);
    // 初始化今天，和前天的时间

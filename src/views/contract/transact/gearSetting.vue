      ...
      <el-form :model="dForm" ref="dForm" :rules="drules">
        ...
        <el-row :span="24">
          <el-form-item label="最高可用杠杆倍数" :label-width="formLabelWidth" prop="maximumLeverage">
            <el-input
              type="number"
              v-model="dForm.maximumLeverage"
              autocomplete="off"
              placeholder=""
              @input="checkVal('maximumLeverage', 'nodot')"
            ></el-input>
          </el-form-item>
        </el-row>
        <b-two-range-choose
          ...
          labelWords="委托张数区间"
          :getVal1.sync="dForm.minPositionAmount"
          :getVal2.sync="dForm.maxPositionAmount"
          :isdisabled="false"
          @handler="getRangeVal"
        ></b-two-range-choose>
        ...
        <el-row :span="24">
          <el-col :span="12">
            <el-form-item label="最低初始保证金率" :label-width="formLabelWidth" prop="minimumInitialMargin">
              <el-input
                type="number"
                v-model="dForm.minimumInitialMargin"
                autocomplete="off"
                placeholder=""
                @input="checkVal('minimumInitialMargin')"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="维持保证金率" :label-width="formLabelWidth" prop="maintenanceMarginRatio">
              <el-input
                v-model="dForm.maintenanceMarginRatio"
                type="number"
                autocomplete="off"
                placeholder=""
                @input="checkVal('maintenanceMarginRatio')"
              >
              </el-input>
            </el-form-item>
          </el-col>
...
export default {
  ...
  methods: {
    ...
    async confirmOp() {
      ...
      this.$refs['dForm'].validate(async valid => {
        if (valid) {
          ...
          const res = id === '' ? await $api.addContractPositionGear(params) : await $api.editContractPositionGear(params);
          if (res) {
    ...
    async getList() {
      ...
      const res = await $api.getContractPositionGearPagination(query_data);
      if (res) {

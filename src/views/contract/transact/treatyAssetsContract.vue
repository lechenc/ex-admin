...
import { treatyAssetsContractCol, treatyAssetsContractConfig } from '@/config/column/contract';
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await $api.getContractAccount(query_data);
      if (res) {
  ...
  mounted() {
    this.configs = treatyAssetsContractCol;
    this.searchCofig = this.$util.clone(treatyAssetsContractConfig);
    // 初始化今天，和前天的时间

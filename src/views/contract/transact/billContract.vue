  ...
  methods: {
    ...
    async calTotal(data) {
      ...
      if (this.search_params_obj.coinId) {
        tmpName = this.symbollist.filter((v) => v['value'] == this.search_params_obj.coinId)[0]
          .label
      } else {
        tmpName = '全部'
      }
      ...
      if (res) {
        ...
        if (getObj) {
          this.$alert(
            `<p>类型：  ${
              this.typeObj[params.type] || '全部'
            }</p>  <p>币对：${tmpName}</p> <p>金额：${getObj.amount}</p>  `,
            '统计结果',
            {
    ...
    requiredParams(params) {
      ...
      if (this.search_params_obj.coinMarket) {
        if (/^[0-9]+.?[0-9]*$/.test(this.search_params_obj.coinMarket)) {
          let tmpName = ''
          tmpName = this.symbollist.filter(

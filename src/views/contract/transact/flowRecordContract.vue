...
import { flowRecordContractCol, flowRecordContractConfig } from '@/config/column/contract';
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await $api.getAccountContractBill(query_data);
      if (res) {
  ...
  mounted() {
    this.configs = flowRecordContractCol;
    this.searchCofig = this.$util.clone(flowRecordContractConfig);
    // 初始化今天，和前天的时间

<template>
  <div class="refundRecord-container">
    ...
    <div class="container-btn" v-if="btnArr && btnArr.length">
      <el-button type="primary" size="medium" v-if="btnArr.includes('edit')" @click="editLine">返佣设置</el-button>
    </div>
    <div>
        ...
        <el-row :span="24">
          <el-col :span="21">
            <el-form-item label="一级返佣比例" :label-width="formLabelWidth" prop="firstLevelRate">
              <el-input type="number" v-model.trim="cForm.firstLevelRate" placeholder="请输入" @input="checkVal('firstLevelRate')">
                <div slot="append">%</div>
              </el-input>
            </el-form-item>
          </el-col>
        ...
        <el-row :span="24">
          <el-col :span="21">
            <el-form-item label="二级返佣比例" :label-width="formLabelWidth" prop="secondLevelRate">
              <el-input type="number" v-model.trim="cForm.secondLevelRate" placeholder="请输入" @input="checkVal('secondLevelRate')">
                <div slot="append">%</div>
              </el-input>
            </el-form-item>
          </el-col>
...
export default {
  ...
  methods: {
    ...
    async calTotal(data) {
      ...
      if (res) {
        ...
        if (getObj) {
          this.$alert(
            `<p>手续费金额：${getObj.originFeeAmountSum}</p>
            <p>平台获得金额：${getObj.platformAmountSum}</p>
            <p>用户返佣金额：${getObj.userAmountSum}</p>`,
            '统计结果',
            {
  ...
  mounted() {
    let authObj = this.$util.getAuthority('RefundRecordNew', [], []);

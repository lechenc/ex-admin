<template>
  <div class="serviceChargeContract-container">
...
import { serviceChargeContractCol, serviceChargeContractConfig } from '@/config/column/contract';
...
export default {
  ...
  methods: {
    ...
    async calTotal(data) {
      ...
      const res = await $api.getContractHandlingFeeTotal(params);
      if (res) {
        ...
        if (getObj) {
          ...
          tmp += `<p>手续费：${getObj.handlingFee || 0}</p>`;
          this.$alert(tmp, '统计结果', {
    ...
    async getList() {
      ...
      const res = await $api.getHandlingFeePagination(query_data);
      if (res) {
  ...
  mounted() {
    this.configs = serviceChargeContractCol;
    this.searchCofig = this.$util.clone(serviceChargeContractConfig);
    // 初始化今天，和前天的时间
...
.serviceChargeContract-container {

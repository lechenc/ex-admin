...
import { userMonitorCol, userMonitorColNoBtn, userMonitorConfig } from '@/config/column/contractAgent';
...
export default {
  ...
  methods: {
    ...
    async doHandle(data) {
      const { fn, row } = data;
      if (fn == 'remove') {
        this.$confirm('是否将该用户移出监控?', '温馨提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning',
        })
          .then(async () => {
            const res = await $api.userMonitorRemove({
              id: row.id,
            });
            if (res) {
      ...
      if (fn == 'supple') {
        ...
        const res = await $api.getMonitorAgentAmount({
    ...
    // 根据查询条件进行合计弹窗展示
    calTotal(data) {
      this.$confirm('是否将全部用户移出监控?', '温馨提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      })
        .then(async () => {
          const res = await $api.userMonitorRemoveAll({});
          if (res) {
            this.$message({ type: 'success', message: res.data.message });
            this.getList();
          }

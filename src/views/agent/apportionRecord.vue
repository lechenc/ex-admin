<template>
  <div class="apportionRecord-container">
...
import { apportionRecordCol, apportionRecordConfig } from '@/config/column/contractAgent';
...
export default {
  ...
  methods: {
    ...
    async calTotal(data) {
      ...
      const res = await $api.apportionRecordSum(params);
      if (res) {
        const getObj = res.data.data;
        if (getObj) {
          this.$alert(
            `<p>已实现盈亏：${getObj.originAmount}</p>
            <p>代理商获得金额：${getObj.amount}</p>`,
            '统计结果',
            {
    ...
    async getList() {
      ...
      const res = await $api.getApportionRecordList(params);
      if (res) {
    ...
    requiredParams(params) {
      ...
      //   // 组件时间初始必须format格式
      //   this.searchCofig[6].value = [befV, nowV];
      // }
  ...
  mounted() {
    ...
    this.configs = apportionRecordCol;
    this.searchCofig = this.$util.clone(apportionRecordConfig);

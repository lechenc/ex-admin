<template>
  <div class="lockManage-container">
    ...
    <div class="sprecon-button" v-if="isCURDAuth">
      ...
      <el-button type="primary" size="medium" @click="addOrder('group')"
        >批量创建锁仓订单</el-button
      >
      <el-button type="primary" size="medium" @click="$router.push('/financial/lockConfig')"
        >锁仓配置</el-button
      >
    </div>
    <div>
    ...
    <el-dialog :title="updateDialog" :visible.sync="addOrderDialog" width="600px">
      <el-form :model="orderForm" :rules="rules" ref="orderForm">
        ...
        <el-form-item label="锁仓数量" :label-width="formLabelWidth" prop="lockAmount">
          <el-input v-model="orderForm.lockAmount" @input="checkVal('lockAmount')" clearable>
          </el-input>
        </el-form-item>
          ...
          <el-row>
            <el-col :span="21">
              <el-input v-model="orderForm.percent" @input="checkVal('percent')" size="small">
                <div slot="prepend">
                  <el-select
                    v-model="orderForm.lockType"
                    placeholder="请选择锁仓时间类型"
                    style="width: 165px"
                  >
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px">
      <el-form :model="dataForm" ref="dataForm" :rules="dataFormRules">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="账户类型" :label-width="formLabelWidth">{{
              curRow.accountType | aStatus
            }}</el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="锁仓类型" :label-width="formLabelWidth">{{
              curRow.orderType | oStatus
            }}</el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="锁仓数量" :label-width="formLabelWidth">{{
              curRow.lockAmount
            }}</el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item label="锁仓时间类型" :label-width="formLabelWidth"
              >{{ curRow.lockType | locktypeDiy }}
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item label="固定释放比例" :label-width="formLabelWidth"
              >{{ curRow.percent }} %</el-form-item
            >
          </el-col>
        </el-row>
        ...
        <el-row v-if="hasChecked">
          <el-col :span="12">
            <el-form-item label="锁仓时间" :label-width="formLabelWidth">{{
              curRow.createTime
            }}</el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="已解冻数量" :label-width="formLabelWidth">{{
              curRow.releaseAmount
            }}</el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="未解冻数量" :label-width="formLabelWidth">{{
              (curRow.lockAmount - curRow.releaseAmount + '').replace(
                /^(\-)*(\d+)\.(\d\d\d\d).*$/,
                '$1$2.$3'
              )
            }}</el-form-item>
  ...
  methods: {
    ...
    confirmAddOrder() {
      ...
      this.$refs['orderForm'].validate(async (valid) => {
        if (valid) {
          const { id, accountType, orderType, coinId, lockType, percent, lockAmount, ...repo } =
            this.orderForm
          let vm = this
          let params = {
    ...
    checkVal(val) {
      if (val === 'percent') {
        ...
        this.orderForm[val] = (this.orderForm[val] + '').replace(
          /^(\-)*(\d+)\.(\d\d).*$/,
          '$1$2.$3'
        ) //两个小数
      }
    ...
    async getList() {
      ...
      if (res) {
        ...
        records.forEach((v) => {
          v.percent =
            v.percent &&
            (Precision.times(v.percent, 100) + '').replace(/^(\-)*(\d+)\.(\d\d).*$/, '$1$2.$3') //只能输入两个小数
          v.amount = v.amount && (v.amount + '').replace(/^(\-)*(\d+)\.(\d\d\d\d).*$/, '$1$2.$3')
          v.remainingAmount =
            v.remainingAmount &&
            (v.remainingAmount + '').replace(/^(\-)*(\d+)\.(\d\d\d\d).*$/, '$1$2.$3')
          v.releaseAmount =
            v.releaseAmount &&
            (v.releaseAmount + '').replace(/^(\-)*(\d+)\.(\d\d\d\d).*$/, '$1$2.$3')
          v.lockAmount =
            v.lockAmount && (v.lockAmount + '').replace(/^(\-)*(\d+)\.(\d\d\d\d).*$/, '$1$2.$3')
        })
        this.list = records

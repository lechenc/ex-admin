...
export default {
  ...
  methods: {
    ...
    async calTotal(data) {
      ...
      if (res) {
        ...
        if (getObj) {
          let coin = this.searchCofig[2]['list'].filter(
            (v) => v.value == this.search_params_obj.coinId
          )[0].label
          this.$alert(
            `<p>币种：${coin}</p><p>增加金额总计：${getObj.addAmountSum}</p><p>减少金额总计：${getObj.subAmountSum}</p>`,
            '统计结果',
            {
  ...
  activated() {
    ...
    this.total = 0 // 总条数
    this.pages = 0 // 总页数
    let authObj = this.$util.getAuthority('CoinFunds', coinFundsCol, [])

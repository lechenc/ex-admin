...
<template>
  <div class="voucherVipUserList-container">
    ...
    <div class="container-btn" v-if="isCURDAuth">
      <el-button type="primary" size="medium" @click="addVip">立即发放尊贵名额</el-button>
    </div>
    <div>
    ...
    <!-- 添加 编辑 -->
    <el-dialog title="发放" width="500px" :visible.sync="dialogFormVisible">
      <el-form :model="cForm" ref="cForm" :rules="rules">
        <el-form-item class="center-item" label="发放尊贵会员邀请名额数量" prop="count" :label-width="labelWidth">
          <el-col :span="24"><el-input @input="checkVal('count', 'noDot')" type="text" placeholder="请输入" v-model="cForm.count"></el-input> </el-col>
        </el-form-item>

        <el-form-item label="UID" prop="uid" :label-width="labelWidth">
          <el-col :span="24"><el-input @input="checkVal2('uid')" type="textarea" placeholder="请输入UID,以逗号隔开" v-model="cForm.uid"></el-input> </el-col>
        </el-form-item>
...
import { voucherVipUserListCol, voucherVipUserListColNoBtn, voucherVipUserListConfig } from '@/config/column/voucher';
...
export default {
  ...
  methods: {
    async doHandle(data) {
      ...
      if (fn === 'details') {
        this.$router.push({
          path: '/front/voucher/voucherVipUserDetails',
          query: {
            superUid: row.uid,
          },
        });
    ...
    async confirmOp() {
      this.$refs['cForm'].validate(async (valid) => {
        if (valid) {
          const { count, uid, googleCode } = this.cForm;
          let params = {
            count,
            uid,
            googleCode,
          ...
          const res = await $api.addVoucherVipUserList(params);
          if (res) {
    ...
    async getList() {
      ...
      const res = await $api.getVoucherVipUserList(params);
      if (res) {

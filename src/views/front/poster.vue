...
<template>
  <div class="notice-container">
    ...
    <div class="notice-button" v-if="isCURDAuth">
      <el-button type="primary" size="medium" @click="createBanner">添加海报</el-button>
    </div>
    <div>
    ...
    <PosterDialog :visible.sync="show_detail_modal" :row.sync="row" :type="show_modal_type" @freshData="getList" @close="closeDialog" />
  </div>
</template>
<script>
import Bsearch from '@/components/search/b-search';
import Btable from '@/components/table/b-table';
import { posterCol, posterColNoBtn, posterConfig } from '@/config/column/front';
...
export default {
  ...
  methods: {
    async doHandle(data) {
      ...
      if (fn === 'delete') {
        this.$confirm('是否删除此海报?', '温馨提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning',
        })
          .then(async () => {
            const res = await $api.deletePosterInfo({
              bannerId: row.id,
            });
            if (res) {
      ...
      if (fn === 'switchCoin') {
        const res = await $api.editPosterInfo({ id: row.id, bannerStatus: row.bannerStatus ? 1 : 0 });
        if (res) {
          this.$message({ type: 'success', message: res.data.message });
          this.getList();
        } else {
    ...
    async getList() {
      ...
      const res = await $api.getPosterList(params);
      if (res) {
        const { data } = res.data;
        data.forEach(v => {
  ...
  mounted() {
    let authObj = this.$util.getAuthority('Poster', posterCol, posterColNoBtn);
    this.configs = authObj.val;
    this.isCURDAuth = authObj.isAdd;

    this.searchConfig = this.$util.clone(posterConfig);
    this.getList();
  },
};
</script>

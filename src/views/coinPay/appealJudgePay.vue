      ...
      <div class="box-card-con">
        <div class="card-info">
          <div class="card-info-panel" v-if="isBuyProEmpty">
            <H3>买方{{ buyProofInfo.isAdvert == 1 ? '广告商' : '' }}申诉信息</H3>
            <div class="line header">
              <span>姓名: {{ buyProofInfo.realName }}</span>
              <span v-if='buyProofInfo.phone'>账号:{{ buyProofInfo.phone }}</span>
            </div>
            <div class="line">
              <div class="title">申诉内容</div>
              <div class="content">{{ buyProofInfo.proofTxt }}</div>
            </div>
            <div class="line">
              <div class="title">申诉凭证</div>
              <div class="img-list" v-if="$util.isArray(buyProofInfo.proofImg) && buyProofInfo.proofImg.length">
                <viewer :options="options" v-for="(item, index) in buyProofInfo.proofImg" :key="index">
                  <img :src="item" alt="" />
                </viewer>
              </div>
          ...
          <div class="card-info-panel" v-if="isSaleProEmpty">
            <H3>卖方{{ saleProofInfo.isAdvert == 1 ? '广告商' : '' }}申诉信息</H3>
            <div class="line header">
              <span>姓名: {{ saleProofInfo.realName }}</span>
              <span v-if="saleProofInfo.phone">账号:{{ saleProofInfo.phone }}</span>
            </div>
            <div class="line">
              <div class="title">申诉内容</div>
              <div class="content">{{ saleProofInfo.proofTxt }}</div>
            </div>
            <div class="line">
              <div class="title">申诉凭证</div>
              <div class="img-list" v-if="$util.isArray(saleProofInfo.proofImg) && saleProofInfo.proofImg.length">
                <viewer :options="options" v-for="(item, index) in saleProofInfo.proofImg" :key="index">
                  <img :src="item" alt="" />
                </viewer>
              </div>
              <div class="img-list" v-else>
      ...
      <div class="box-card-con">
        ...
        <el-form>
          <el-form-item label="裁决结果">
            <div>{{ resultRow.type | winFilter }}</div>
          </el-form-item>
          <el-form-item label="客服处理普通用户结果">
            <div>{{ resultRow.buyerResult | buyerFilter }}</div>
          </el-form-item>
          <el-form-item label="客服处理广告商结果">
            <div>{{ resultRow.sellerResult | sellerFilter }}</div>
          </el-form-item>
          <el-form-item label="备注">
            <div>{{ resultRow.auditComment }}</div>
          </el-form-item>
          <el-form-item label="处理时间">
            <div>{{ resultRow.auditTime | typeTime }}</div>
          </el-form-item>
          <el-form-item label="处理人">
            <div>{{ resultRow.auditUserName }}</div>
          </el-form-item>
          <el-form-item label="处理人IP">
            <div>{{ resultRow.auditIp }}</div>
          </el-form-item>
        </el-form>
      </div>
    </el-card>
    ...
    <el-card class="box-card" v-if="pageStatus === 'judgment'">
      <div class="box-card-con">
        ...
        <el-form :model="ruleForm" ref="ruleForm" :rules="rules">
          <!-- <el-row> -->
            <!-- <el-col :span="10"> -->
              <el-form-item label="裁决结果" prop="appealResult" label-width="120px">
                <el-select v-model="ruleForm.appealResult" placeholder="请选择" style="min-width: 320px" :disabled="!isModify">
                  <el-option v-for="(item, idx) in judgmentList" :key="idx" :label="item.label" :value="item.value"></el-option>
                </el-select>
              </el-form-item>
            <!-- </el-col> -->
            <!-- <el-col :span="7"> -->
              <el-form-item label="处罚普通用户" prop="userResult" label-width="120px">
                <el-select v-model="ruleForm.userResult" placeholder="请选择" :disabled="!isModify">
                  <el-option v-for="(item, idx) in punishBuyList" :key="idx" :label="item.label" :value="item.value"></el-option>
                </el-select>
              </el-form-item>
            <!-- </el-col> -->
            <!-- <el-col :span="7"> -->
              <el-form-item label="处罚广告商" prop="advertResult" label-width="120px">
                <el-select v-model="ruleForm.advertResult" placeholder="请选择" :disabled="!isModify">
          ...
          <!-- </el-row> -->
          <el-row>
            <el-form-item label="备注" prop="auditComment" label-width="120px">
              <el-input type="textarea" rows="4" placeholder="请输入" v-model="ruleForm.auditComment" :disabled="!isModify"> </el-input>
            </el-form-item>
          </el-row>
        </el-form>
      </div>
...
import { appealDetailCol } from '@/config/column/coinPay';
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        appealResult: [{ required: true, message: '裁决结果不能为空', trigger: 'blur' }],
        userResult: [{ required: true, message: '普通用户判决不能为空', trigger: 'blur' }],
        advertResult: [{ required: true, message: '广告商判决不能为空', trigger: 'blur' }],
        auditComment: [{ required: true, message: '备注不能为空', trigger: 'blur' }],
      },
      ...
      // 裁决结果列表（appealResult）
      judgmentList: [{ label: '买方胜诉', value: 2 }, { label: '卖方胜诉', value: 1 }],
      // 处罚普通用户列表（userResult）
      punishBuyList: [{ label: '不惩罚', value: 1 }, { label: '禁止法币交易', value: 2 }],
      // 处罚广告商列表（advertResult）
      punishSellList: [{ label: '不惩罚', value: 1 }, { label: '禁止发布广告', value: 3 }],
      options: {
        inline: false,
        button: false,
        navbar: false,
  ...
  methods: {
    ...
    async appealJudgment() {
      this.$refs['ruleForm'].validate(async valid => {
        if (valid) {
          ...
          this.$confirm(desc, title, { confirmButtonText: '确定', cancelButtonText: '取消', center: true })
            .then(async () => {
              const { id, ...repo } = this.ruleForm;
              let params = {
                id: this.appealId,
                ...repo,
              };
              this.btnLoading = true;
              const res = await $api.putaAdjustAppealNew(params);
              if (res) {
                this.$message({ message: '裁决成功', type: 'success' });
                this.isModify = false;
                // 把详情页和列表页从keep-alive中缓存,保证返回后列表页数据状态是最新的
                this.reloadTagsPage('/coinPay/appealJudgePay');
                this.reloadTagsPage('/coinPay/appealPay');
                // this.getBuyList()
              }
              this.btnLoading = false;
              this.$router.go(-1);
    ...
    async getDetail(_id) {
      ...
      if (res) {
        ...
        this.buyProofInfo.proofImg = this.dealPic(this.buyProofInfo.proofImg);
        this.isBuyProEmpty = !this.$util.isEmptyObject(this.buyProofInfo) ? true : false;
        this.saleProofInfo = res.data.data.sellAppeal;
        this.saleProofInfo.proofImg = this.dealPic(this.saleProofInfo.proofImg);
    ...
    initFunc() {
      ...
      this.appealStatus = this.$route.query.appealStatus;
      // this.curRow = JSON.parse(this.$route.query.row);

 ...
 * @LastEditTime: 2020-12-01 20:15:26
...
<template>
  <div class="extractPay-container">
...
import { extractCol, extractColNoBtn, extractConfig } from '@/config/column/coinPay';
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await $api.getCPWithdrawQueryList(query_data);
      if (res) {
    ...
    async queryData(params) {
      ...
      this.requiredParams(params);
      Object.assign(params, this.search_params_obj);
      const res = await $api.getCPWithdrawQueryList(params);
  ...
  activated() {
    ...
    // 可能从资产管理》收入账号页面跳入传参
    let toAddress = this.$route.query.toAddress;
    let coinName = this.$route.query.coinName;
    let coinId = this.$route.query.coinId;
    if (toAddress) {
      // 从列表页传参
      this.searchCofig[3].value = toAddress;

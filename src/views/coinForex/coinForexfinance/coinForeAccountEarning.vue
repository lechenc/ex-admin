 ...
 * @FilePath: \mt4-statisticsd:\阿尔法项目\alphawallet-bg\src\views\symbol\setting.vue
 -->
<template>
  <div class="coinForeAccountEarning-container">
...
import { coinForeAccountEarningCol, coinForeAccountEarningConfig } from '@/config/column/coinForex'
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await $api.apiGetCoinForeAccountEarningList(query_data)
      if (res) {
  ...
  async mounted() {
    this.configs = coinForeAccountEarningCol
    this.searchCofig = this.$util.clone(coinForeAccountEarningConfig)
...
<style lang="scss">
.coinForeAccountEarning-container {

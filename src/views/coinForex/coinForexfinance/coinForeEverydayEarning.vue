 ...
 * @FilePath: \mt4-statisticsd:\阿尔法项目\alphawallet-bg\src\views\symbol\setting.vue
 -->
<template>
  <div class="coinForeEverydayEarning-container">
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await $api.apiGetCoinForeEverydayEarningList(query_data)
      if (res) {
  ...
  async mounted() {
    this.configs = coinForeEverydayEarningCol
    this.searchCofig = this.$util.clone(coinForeEverydayEarningConfig)
...
<style lang="scss">
.coinForeEverydayEarning-container {

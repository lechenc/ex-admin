...
export default {
  ...
  methods: {
    ...
    async calTotal(data) {
      ...
      const res = await $api.getContractEntrustDetailPaginationTotal(params);
      if (res) {
        const getObj = res.data.data;
        if (getObj) {
          let tmp = '';
          if (this.search_params_obj.direction) {
            const tm1 = this.searchCofig[7]['list'].filter(v => v.value == this.search_params_obj.direction)[0].label;
            tmp += `<p>成交方向：${tm1}</p>`;
          }
          ...
          tmp += `<p>成交量：${getObj.amount}</p><p>成交均价：${getObj.averagePrice || 0}</p>`;
          this.$alert(tmp, '统计结果', {
    ...
    async getList() {
      ...
      const res = await $api.getCloseEntrustDetailPaginationMock(query_data);
      if (res) {
    ...
    async getSymbolList() {
      ...
      this.$store.dispatch('common/getSymbolMimicListAnalystAll').then(() => {
        ...
        this.searchCofig[3]['list'] = this.coin_List;
      });
    },
  },
  mounted() {

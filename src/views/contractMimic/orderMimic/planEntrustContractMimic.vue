...
export default {
  ...
  methods: {
    ...
    async calTotal(data) {
      ...
      const res = await $api.getContractPlanEntrustTotal(params);
      if (res) {
        ...
        if (getObj) {
          ...
          tmp += `<p>委托量：${getObj.entrustDeposit}</p><p>保证金：${getObj.entrustAmount || 0}</p>`;
          this.$alert(tmp, '统计结果', {
    ...
    async getList() {
      ...
      const res = await $api.getContractPlanEntrustPaginationMock(query_data);
      if (res) {

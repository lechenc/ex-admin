...
import { treatyAssetsContractMimicCol, treatyAssetsContractMimicConfig } from '@/config/column/contractMimic';
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await $api.getContractAccountMock(query_data);
      if (res) {
  ...
  mounted() {
    this.configs = treatyAssetsContractMimicCol;
    this.searchCofig = this.$util.clone(treatyAssetsContractMimicConfig);
    // 初始化今天，和前天的时间

<template>
  <div class="flowRecordContract-container">
...
import { flowRecordContractMimicCol, flowRecordContractMimicConfig } from '@/config/column/contractMimic';
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await $api.getAccountContractBillMock(query_data);
      if (res) {
  ...
  mounted() {
    this.configs = flowRecordContractMimicCol;
    this.searchCofig = this.$util.clone(flowRecordContractMimicConfig);
    // 初始化今天，和前天的时间

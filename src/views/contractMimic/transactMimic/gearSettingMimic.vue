...
<template>
  <div class="coinContract-container">
    ...
    <div class="container-btn">
      <el-button type="primary" size="small" @click="addGear">添加</el-button>
    </div>
    <div>
    ...
    <el-dialog :title="formName" width="670px" :visible.sync="dialogFormVisible">
      <el-form :model="dForm" ref="dForm" :rules="drules">
        ...
        <el-row :span="24">
          <el-form-item label="档位选择" :label-width="formLabelWidth" prop="gear">
            <el-input type="number" v-model="dForm.gear" autocomplete="off" placeholder="" @input="checkVal('gear', 'nodot')"></el-input>
          </el-form-item>
        </el-row>
        <el-row :span="24">
          <el-form-item label="最高可用杠杆倍数" :label-width="formLabelWidth" prop="maximumLeverage">
            <el-input type="number" v-model="dForm.maximumLeverage" autocomplete="off" placeholder="" @input="checkVal('maximumLeverage', 'nodot')"></el-input>
          </el-form-item>
        </el-row>
        <b-two-range-choose v-if="true" ref="twoChoose" :labelWidth="formLabelWidth" labelWords="委托张数区间" :getVal1.sync="dForm.minPositionAmount" :getVal2.sync="dForm.maxPositionAmount" :isdisabled="false" @handler="getRangeVal"></b-two-range-choose>

        <el-row :span="24">
          <el-col :span="12">
            <el-form-item label="最低初始保证金率" :label-width="formLabelWidth" prop="minimumInitialMargin">
              <el-input type="number" v-model="dForm.minimumInitialMargin" autocomplete="off" placeholder="" @input="checkVal('minimumInitialMargin')"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="维持保证金率" :label-width="formLabelWidth" prop="maintenanceMarginRatio">
              <el-input v-model="dForm.maintenanceMarginRatio" type="number" autocomplete="off" placeholder="" @input="checkVal('maintenanceMarginRatio')"> </el-input>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :span="24">
          <el-form-item label="是否默认" :label-width="formLabelWidth" prop="isDefault">
            <el-switch v-model="dForm.isDefault" active-color="#13ce66" inactive-color="#ff4949"> </el-switch>
          </el-form-item>
        </el-row>
      </el-form>
...
import { gearSettingCol, gearSettingConfig } from '@/config/column/contract';
...
export default {
  ...
  data() {
    return {
      ...
      drules: {
        ...
        gear: [{ required: true, message: '必填', trigger: 'blur' }],
        // coinMarket: [{ required: true, message: '必填', trigger: 'blur' }],
        maximumLeverage: [{ required: true, message: '必填', trigger: 'blur' }],
        // minPositionAmount: [{ required: true, message: '必填', trigger: 'blur' }],
        // maxPositionAmount: [{ required: true, message: '必填', trigger: 'blur' }],
        minimumInitialMargin: [{ required: true, message: '必填', trigger: 'blur' }],
        maintenanceMarginRatio: [{ required: true, message: '必填', trigger: 'blur' }],
      },
    };
  },
  methods: {
    ...
    async confirmOp() {
      ...
      this.$refs['dForm'].validate(async (valid) => {
        if (valid) {
          ...
          const res = id === '' ? await $api.addContractPositionGearMock(params) : await $api.editContractPositionGearMock(params);
          if (res) {
    ...
    async doHandle(data) {
      ...
      if (fn === 'edit') {
        ...
        this.$nextTick(() => {
          this.$refs['dForm'].resetFields();
          let { id, gear, isDefault, coinMarket, maximumLeverage, minPositionAmount, maxPositionAmount, minimumInitialMargin, maintenanceMarginRatio } = row;
          this.dForm = {
            id,
            gear,
            coinMarket,
      ...
      if (fn === 'trstart') {
        ...
        const res = await $api.editContractPositionGearMock(params);
        if (res) {
    ...
    async getList() {
      ...
      const res = await $api.getContractPositionGearPaginationMock(query_data);
      if (res) {
  ...
  async mounted() {
    this.configs = gearSettingCol;
    this.searchCofig = this.$util.clone(gearSettingConfig);

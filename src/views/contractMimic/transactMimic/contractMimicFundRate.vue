      ...
      <el-form :model="dForm" ref="dForm" :rules="drules">
        ...
        <el-row :span="24">
          <el-form-item label="预计资金费率" :label-width="formLabelWidth" prop="estimatedCapitalRate">
            <el-input type="text" v-model="dForm.estimatedCapitalRate" autocomplete="off" placeholder=""></el-input>
          </el-form-item>
        </el-row>
      </el-form>
...
import { contractFundRateMimicCol, contractFundRateMimicConfig } from '@/config/column/contractMimic';
...
export default {
  ...
  methods: {
    ...
    async confirmOp() {
      this.$refs['dForm'].validate(async (valid) => {
        if (valid) {
          ...
          const res = await $api.editContractFundRateMock(params);
          if (res) {
    ...
    async getList() {
      ...
      const res = await $api.getContractFundRateListMock(query_data);
      if (res) {
  ...
  async mounted() {
    this.configs = contractFundRateMimicCol;
    this.searchCofig = this.$util.clone(contractFundRateMimicConfig);

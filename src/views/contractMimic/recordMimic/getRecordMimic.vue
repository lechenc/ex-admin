...
import { getRecordMimicCol, getRecordMimicConfig } from '@/config/column/contractMimic';
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await $api.getRecordMimicList(query_data);
      if (res) {
  ...
  mounted() {
    this.configs = getRecordMimicCol;
    this.searchCofig = this.$util.clone(getRecordMimicConfig);
    // 初始化今天，和前天的时间

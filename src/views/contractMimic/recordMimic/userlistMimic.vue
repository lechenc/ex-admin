...
import { userlistMimicCol, userlistMimicConfig } from '@/config/column/contractMimic';
...
export default {
  ...
  methods: {
    async doHandle(data) {
      ...
      if (fn == 'receiveSwitch') {
        ...
        const params = {
          userId: row.userId,
          receiveSwitch: row.receiveSwitch ? 1 : 0,
        };
        const res = await $api.changeUserStatusMimic(params);
        if (res) {
    ...
    async getList() {
      ...
      this.listLoading = true;
      const res = await $api.getUserListMimic(params);
      if (res) {
        ...
        records.forEach((v) => {
          v['receiveSwitch'] = v['receiveSwitch'] ? true : false;
        });
        this.list = records;
  ...
  mounted() {
    ...
    this.searchCofig = this.$util.clone(userlistMimicConfig);
    this.getList();
  },
};
</script>

<template>
  <div class="revenueAccount-container">
    <div class="container-btn">
      <el-button type="primary" size="medium" @click="addLine('')" v-if="btnHeaderArr.includes('manual')">手动充值</el-button>
      <el-button type="primary" size="medium" @click="addLine('transfer')" v-if="btnHeaderArr.includes('add')">划转</el-button>
    </div>
    <div>
    ...
    <el-dialog :visible.sync="dialogVisible" width="500px" :title="title">
      <el-form :model="form" ref="form" :rules="rules">
        ...
        <el-form-item :label="isTransfer ? '转出数量' : '充值数量'" prop="amount" :label-width="formLabelWidth">
          <el-input type="number" placeholder="请输入" v-model="form.amount" @input="checkVal('amount')"> </el-input>
        </el-form-item>
        <el-form-item label="接收账户" :label-width="formLabelWidth" prop="toUserId" v-if="isTransfer">
...
import { revenueAccountCol, revenueAccountColNoBtn, revenueAccountConfig } from '@/config/column/assetManage';
...
export default {
  ...
  methods: {
    async doHandle(data) {
      ...
      if (fn === 'manualEdit') {
        // 手动充值记录
        this.$router.push({ path: '/assetManage/manualRechargeRecord', query: { coinId: row.coinId, coinName: row.coinName } });
      } else if (fn === 'transferEdit') {
        //  划转记录
        this.$router.push({ path: '/assetManage/transferRecord', query: { uid: row.uid } });
      } else if (fn === 'autoEdit') {
        //  自动充值记录（跳转充币记录页面）
        this.$router.push({ path: '/financial/recharge', query: { uid: row.uid } });
      }else if(fn === 'transferIn'){
        // 站内转入记录
        const {curData} = data;
         this.$router.push({ path: '/financial/extract', query: { toAddress: curData, coinId: row.coinId,coinName:row.coinName } });
      }
    },
    doSearch(data) {
      this.current_page = 1;
    ...
    async getTransferUserList(val) {
      ...
      if (res.data && res.data.data) {
        ...
        this.accountList = tmp.map(v => {
          return { label: v.realName, value: v.userId, typeId: v.userType };
        });
      }
    },
    // 获取该币种的余额
    ...
    confirmOp: _.debounce(function() {
      ...
      this.$refs['form'].validate(async valid => {
        if (valid) {
          ...
          const res = this.isTransfer ? await $api.setIncomeUserTransfer(params) : await $api.getRechargeIncomeIn(params);
          if (res) {
            this.dialogVisible = false;
            let txt = this.isTransfer ? '划转成功' : '手动充值成功';
    ...
    async getList() {
      ...
      const res = await $api.getAccountIncomeDetail(params);
      if (res.data && res.data.data) {
    ...
    initFunc() {
      let authObj = this.$util.getAuthority('RevenueAccount', revenueAccountCol, revenueAccountColNoBtn);
      this.configs = authObj.val;
      // this.isCURDAuth = authObj.isAdd;
      this.btnHeaderArr = authObj.btnArr || []; // 页顶的（划转和手动充值按钮根据获取的按钮配置检测有无来显隐）

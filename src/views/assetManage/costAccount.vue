<template>
  <div class="costAccount-container">
...
import { costAccountCol, costAccountColNoBtn, costAccountConfig } from '@/config/column/assetManage';
...
export default {
  ...
  methods: {
    ...
    confirmOp() {
      this.$refs['form'].validate(async valid => {
        if (valid) {
          ...
          const res = await $api.setCostUserTransfer(params);
          if (res) {
    ...
    async getList() {
      ...
      const res = await $api.getCostDetail(params);
      if (res) {
        this.list = res.data && res.data.data ? res.data.data : [];
      }
    ...
    initFunc() {
      let authObj = this.$util.getAuthority('CostAccount', costAccountCol, costAccountColNoBtn);
      this.configs = authObj.val;
      this.isCURDAuth = authObj.isAdd;

      this.searchCofig = this.$util.clone(costAccountConfig);

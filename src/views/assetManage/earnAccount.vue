<template>
  <div class="earnAccount-container">
    <div class="container-btn">
      <el-button type="primary" size="medium" @click="addLine('transfer')" v-if="btnHeaderArr.includes('writeoff')">划转</el-button>
      <el-button type="primary" size="medium" @click="addLine('')" v-if="btnHeaderArr.includes('add')">销账</el-button>
    </div>
    <div>
    ...
    <el-dialog :visible.sync="dialogVisible" width="500px" :title="title">
      <el-form :model="form" ref="form" :rules="rules">
        ...
        <el-form-item :label="isTransfer ? '转出数量' : '销账数量'" prop="amount" :label-width="formLabelWidth">
...
import { earnAccountCol, earnAccountColNoBtn, earnAccountConfig } from '@/config/column/assetManage';
...
export default {
  ...
  methods: {
    async doHandle(data) {
      ...
      if (fn === 'earnEdit') {
        // 收益记录
        this.$router.push({ path: '/assetManage/earnRecord', query: { coinId: row.coinId, coinName: row.coinName } });
      } else if (fn === 'transferEdit') {
        //  划转记录
        this.$router.push({ path: '/assetManage/transferRecord', query: { uid: row.uid } });
      } else if (fn === 'writeoffEdit') {
        //  销账记录
        this.$router.push({ path: '/assetManage/writeoffRecord', query: { uid: row.uid } });
      }
    },
    doSearch(data) {
      this.current_page = 1;
    ...
    confirmOp: _.debounce(function() {
      ...
      this.$refs['form'].validate(async valid => {
        if (valid) {
          ...
          const res = this.isTransfer ? await $api.addProfitUserTransfer(params) : await $api.getWriteOffProfit(params);
          if (res) {
            this.dialogVisible = false;
            let txt = this.isTransfer ? '划转成功' : '销账成功';
    ...
    async getList() {
      ...
      const res = await $api.getProfitDetail(params);
      if (res.data && res.data.data) {
    ...
    initFunc() {
      let authObj = this.$util.getAuthority('EarnAccount', earnAccountCol, earnAccountColNoBtn);
      this.configs = authObj.val;
      // this.isCURDAuth = authObj.isAdd;
      this.btnHeaderArr = authObj.btnArr || [];

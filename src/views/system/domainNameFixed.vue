<template>
  <div class="domainNameFixed-container">
...
import { domainNameFixedCol, domainNameFixedColNoBtn } from '@/config/column/system';
...
export default {
  ...
  methods: {
    ...
    async doHandle(data) {
      ...
      if (fn === 'del') {
        this.$confirm('确定删除？', '温馨提示', {
          ...
          .then(async () => {
            ...
            const res = await $api.apiSetDeleteDomainNameFixed(params);
            if (res) {
              this.$message({
                type: 'success',
                message: '删除成功!',
      ...
      if (fn === 'trDefault') {
        ...
        const res = await $api.apiSetUpdateDomainNameFixed(params);
        if (res) {
          this.$message({
            type: 'success',
            message: '操作成功!',
    ...
    confirmOp() {
      this.$refs['chainForm'].validate(async (valid) => {
        if (valid) {
          ...
          const res =
            id === ''
              ? await $api.apiSetAddDomainNameFixed(params)
              : await $api.apiSetUpdateDomainNameFixed({
                  id,
                  ...params,
                });
          if (res) {
    ...
    async getList() {
      ...
      const res = await $api.apiGetDomainNameFixedList(query_data);
      if (res) {
  ...
  async mounted() {
    let authObj = this.$util.getAuthority('DomainNameFixed', domainNameFixedCol, domainNameFixedColNoBtn);

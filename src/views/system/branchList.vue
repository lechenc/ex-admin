 ...
 * @FilePath: \mt4-statisticsd:\阿尔法项目\alphawallet-bg\src\views\system\branchList.vue
 -->
<template>
  <div class="branchList-container">
    <div class="container-top">
      <Bsearch :configs="searchCofig" @do-search="doSearch" @do-reset="doReset" />
    </div>

    <div class="container-btn" v-if="isCURDAuth">
      <el-button type="primary" size="medium" @click="addbranchList">添加角色</el-button>
    </div>
    <div>
    ...
    <el-dialog :title="formName" :visible.sync="dialogFormVisible">
      <el-form :model="branchListForm" ref="branchListForm" :rules="rules">
        <el-form-item label="角色名称" :label-width="formLabelWidth" prop="name">
          <el-input v-model="branchListForm.name" autocomplete="off"></el-input>
        </el-form-item>
...
import { branchListCol, branchListColNoBtn, branchListConfig } from '@/config/column/system';
...
export default {
  ...
  methods: {
    ...
    async doHandle(data) {
      ...
      if (fn === 'edit') {
        this.formName = '编辑';
        this.branchListForm.id = row.id;
        this.branchListForm.name = row.name;
      ...
      if (fn === 'delete') {
        this.$confirm('确定删除？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消' })
          .then(async () => {
            const res = await $api.deletebranchList({ id: row.id });
            if (res) {
              this.$message({
                message: '删除角色成功',
                type: 'success',
          ...
          .catch(() => {
            // console.log('deletebranchList error');
          });
      }
    },
    // 分页
  ...
  mounted() {
    let authObj = this.$util.getAuthority('BranchList', branchListCol, branchListColNoBtn);
    this.configs = authObj.val;
    this.isCURDAuth = authObj.isAdd;

    this.searchCofig = this.$util.clone(branchListConfig);
    this.getList();
    this.getMenuInfo();
  },
};

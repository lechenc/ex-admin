  ...
  <div class="assetStatistic-container">
    <el-tabs v-model="activeName">
      <el-tab-pane label="实时总资产" name="now">
        ...
        <template>
          <Btable :listLoading="listLoading" :data="list" :configs="configs1" />
        </template>
      </el-tab-pane>
      <el-tab-pane label="历史资产统计" name="past">
        <div class="container-top">
          <Bsearch :configs="searchCofig" @do-search="doSearch" @do-reset="doReset" calText="资产统计" :calTotal="true" @do-calTotal="calTotal" />
        </div>
        <Btable :listLoading="listLoading" :data="listPast" :configs="configs2" />
  ...
  methods: {
    ...
    async calTotal(data) {
      ...
      if (res) {
        ...
        const tmpName = this.coinList.filter(v => v['value'] == this.search_params_obj.coinId)[0].label;
        if (getObj) {

<template>
  <div class="tradelistspecial-container">
  ...
  methods: {
    ...
    async calTotal(data) {
      ...
      if (res) {
        ...
        if (getObj) {
          this.$alert(
            `<p>交易对：${this.search_params_obj.coinMarket}</p><p>成交量总计：${getObj.amount}</p><p>成交均价：${getObj.averagePrice}</p>`,
            '统计结果',
            {

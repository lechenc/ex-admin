 ...
 * @LastEditTime: 2020-09-27 16:01:21
  ...
  <div class="orderlist-container">
    ...
    <!-- <el-dialog :visible.sync="show_detail_modal" :fullscreen="true">
      <el-card class="box-card">
        <h3>{{ row.type === 1 ? "买入" : "卖出" }}{{ row.symbol }}</h3>
        <div class="box-card-con">
          <div>账号：{{ row.phone }}</div>
          <div>委托价：{{ row.price }}</div>
          <div>委托量：{{ row.amount }}</div>
          <div>委托状态：{{ row.status == 0 ? "挂单中" : row.status == 1 ? "已撤单" : row.status == 2 ? "已成交" : "部分成交" }}</div>
          <div>委托类型：{{ row.priceType == 0 ? "限价" : "市价" }}</div>
          <div>成交量：{{ row.dealAmount }}</div>
          <div>手续费：{{ row.exFee }}</div>
          <div>账号类型：{{ row.userType == 1 ? "客户" : "操盘手" }}</div>
          <div>委托时间：{{ row.createTime }}</div>
        </div>
      </el-card>
      <h3>成交信息:</h3>
...
export default {
  ...
  methods: {
    ...
    async calTotal(data) {
      ...
      if (this.search_params_obj.maxPrice < this.search_params_obj.minPrice) {
        this.$message.error('委托价格区间最小值不得大于最大值');
        return;
      }
      // 合计统计查询时，委托状态必须设置为“待成交”，pm规定
      this.searchCofig[6]['value'] = 0;
      if (this.search_params_obj.maxPrice || this.search_params_obj.minPrice) {
        if (!(this.search_params_obj.maxPrice && this.search_params_obj.minPrice)) {
          this.$message({ type: 'error', message: '委托价格区间最小值和最大值必填!', duration: 2500 });
          return;
        }
      }
      ...
      if (res) {
        ...
        if (getObj) {
          ...
          let tmp = `<p>委托方向：${tm1}</p><p>委托类型：${tm2}</p><p>交易对：${this.search_params_obj.coinMarket}</p><p>委托量总计：${
            getObj.amount
          }</p><p>挂单金额总计：${getObj.onAmount || 0}</p>`;
          if (this.search_params_obj.maxPrice || this.search_params_obj.minPrice) {
            tmp = `<p>委托方向：${tm1}</p><p>委托类型：${tm2}</p><p>交易对：${this.search_params_obj.coinMarket}</p>
            <p>委托价格区间：${this.search_params_obj.minPrice} ~ ${this.search_params_obj.maxPrice}</p>
            <p>委托量总计：${getObj.amount}</p><p>挂单金额总计：${getObj.onAmount || 0}</p>`;
          }
          this.$alert(tmp, '统计结果', {
    ...
    summaryMethod({ columns, data }) {
      const sums = [];
      const sumKeys = ['price', 'amount', 'dealAmount', 'averagePrice', 'onAmount'];
    ...
    async getList() {
      ...
      if (res) {
        ...
        this.list = records.map(el => {
          el.entrustAmount = Precision.times(el.price, el.amount);
          return el;
        });
      }

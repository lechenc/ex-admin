<template>
  <div class="tradelist-container">
    <el-row class="sac-row" v-if="topShow">
      <el-col :span="4">
...
import { tradeCol, tradeConfig } from '@/config/column/exchange';
...
export default {
  ...
  methods: {
    ...
    async calTotal(data) {
      ...
      const res = await $api.getOrderEntrustDetailTotal(params);
      if (res) {
        const getObj = res.data.data;
        if (getObj) {
          this.$alert(`<p>交易对：${this.search_params_obj.coinMarket}</p><p>成交量总计：${getObj.amount}</p><p>成交均价：${getObj.averagePrice}</p>`, '统计结果', {
  ...
  mounted() {
    this.configs = tradeCol;
    this.searchCofig = this.$util.clone(tradeConfig);
    // 初始化今天，和前天的时间
    this.toDay = this.$util.diyTime('toDay');
    this.ago = this.$util.diyTime('ago');

    if (this.$route.query.uid) {
      this.search_params_obj.coinMarket = this.$route.query.coinMarket;
      this.search_params_obj.buyUid = this.$route.query.uid;
      this.searchCofig[1].value = this.$route.query.uid;
      this.searchCofig[5].value = this.$route.query.coinMarket;
      this.topShow = true;
    }
    this.getList();
    this.getSymbolList();

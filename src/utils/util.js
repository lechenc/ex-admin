...
const datePickerOptions = (options = { disabledDate: 'aft' }) => {
  ...
  return {
    ...
    disabledDate(date) {
      ...
      overT.setMinutes(59)
      overT.setSeconds(59)
      return options.disabledDate === 'aft'
        ? +date > +overT
        : options.disabledDate === 'pre'
  ...
  if (type === 0) {
    ...
    const loadData = async (total, page = 1, per = 100) => {
      try {
        ...
        dataCache = [...dataCache, ...(res.data.data.records || res.data.data || [])]
    ...
    try {
      ...
      if (exportnumber > max) {
        ...
        const confirm = await that.$confirm(
          `当前条件总数据${exportnumber}, 是否仅导出前${max}条? 若导出，请耐心等待进度条即可，请尽量不要中途关闭退出。`,
          '导出信息确认',
          {
...
function getAuthority(name, vArr, noVarr) {
  ...
  if (matchVal && matchVal.length > 0) {
    ...
    if (_vArr) {
      ...
      _vArr = _vArr.filter(
        v => v.type != 'switch' || (v.type == 'switch' && trArr.includes(v.alias))
      )
